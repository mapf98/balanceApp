module.exports = {
  ...
  getTransaction: function(con, id, callback) {
    con.query(
      "SELECT * FROM TRANSACCION WHERE transaction_id = ? ",
      [id],
      callback
    );
  },
  ...
  updateTransaction: function(con, id, body, callback) {
    con.query(
      "UPDATE TRANSACCION SET transaction_account_total = ?, transaction_amount = ?, transaction_concept = ?, transaction_date = ?, fk_account_id = ?, fk_category_id = ? WHERE transaction_id = ?;",
      [
        body.transaction_account_total,
        body.transaction_amount,
        body.transaction_concept,
  ...
  deleteTransaction: function(con, id, callback) {
    con.query(
      "DELETE FROM TRANSACCION WHERE transaction_id = ?;",
      [id],
      callback
    );
  }
